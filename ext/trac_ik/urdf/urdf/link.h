...
struct Material {
  ...
  Material(const Material& m)
      : name(m.name), texture_filename(m.texture_filename), color(m.color) {}
...
struct Collision {
  ...
  Collision(const Collision& c)
      : name(c.name), origin(c.origin), geometry(c.geometry) {}
