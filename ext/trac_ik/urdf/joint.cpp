...
const char* getParentJointName(TiXmlElement* xml) {
  // this should always be set since we check for the joint name in parseJoint
  // already
...
std::shared_ptr<JointDynamics> JointDynamics::fromXml(TiXmlElement* xml) {
  ...
  if (damping_str == NULL && friction_str == NULL) {
    std::ostringstream error_msg;
    error_msg << "Error while parsing joint '" << getParentJointName(xml)
              << "': joint dynamics element specified with no damping and no "
                 "friction!";
    throw URDFParseError(error_msg.str());
  }
...
std::shared_ptr<Joint> Joint::fromXml(TiXmlElement* xml) {
  ...
  if (origin_xml != NULL) {
    try {
      ...
      std::ostringstream error_msg;
      error_msg << "Error! Malformed parent origin element for joint '"
                << joint->name << "': " << e.what() << "!";
      throw URDFParseError(error_msg.str());
    }
  }
  ...
  if (type_str == "planar")
    ...
    std::ostringstream error_msg;
    error_msg << "Error! Joint '" << joint->name << "' has unknown type ("
              << type_str << ")!";
    throw URDFParseError(error_msg.str());
  }
    ...
    if (axis_xml == NULL) {
      ...
      if (axis_xml->Attribute("xyz")) {
        try {
          ...
          std::ostringstream error_msg;
          error_msg << "Error! Malformed axis element for joint ["
                    << joint->name << "]: " << e.what();
          throw URDFParseError(error_msg.str());
        }
      }
