...
void UrdfModel::initLinkTree(map<string, string>& parent_link_tree) {
  for (auto joint = joint_map.begin(); joint != joint_map.end(); joint++) {
    ...
    if (child_link == nullptr) {
      ostringstream error_msg;
      error_msg << "Error while constructing model! Child link ["
                << child_link_name << "] of joint [" << joint->first
                << "] not found";
      throw URDFParseError(error_msg.str());
    }
    ...
    if (parent_link == nullptr) {
      ostringstream error_msg;
      error_msg << "Error while constructing model! Parent link ["
                << parent_link_name << "] of joint [" << joint->first
                << "] not found";
      throw URDFParseError(error_msg.str());
    }
...
void UrdfModel::findRoot(const map<string, string>& parent_link_tree) {
  ...
  if (root_link == nullptr) {
    throw URDFParseError(
        "Error! No root link found. The urdf does not contain a valid link "
        "tree.");
  }
}

std::shared_ptr<UrdfModel> UrdfModel::fromUrdfStr(
  ...
  if (name != nullptr) {
    ...
    std::string error_msg =
        "No name given for the robot. Please add a name attribute to the robot "
        "element!";
    throw URDFParseError(error_msg);
  }

  for (TiXmlElement* material_xml = robot_xml->FirstChildElement("material");
       material_xml != nullptr;
       material_xml = material_xml->NextSiblingElement("material")) {
    auto material = Material::fromXml(
        material_xml, false);  // material needs to be fully defined here
  ...
  for (TiXmlElement* link_xml = robot_xml->FirstChildElement("link");
       link_xml != nullptr; link_xml = link_xml->NextSiblingElement("link")) {
        ...
        for (auto visual : link->visuals) {
          if (!visual->material_name.empty()) {
            if (model->getMaterial(visual->material_name) != nullptr) {
              visual->material.emplace(
                  model->getMaterial(visual->material_name.c_str()));
            } else {
              // if no model matrial found use the one defined in the visual
              if (visual->material.has_value()) {
                model->material_map[visual->material_name] =
                    visual->material.value();
              } else {
                // no matrial information available for this visual -> error
                std::ostringstream error_msg;
                error_msg << "Error! Link '" << link->name << "' material '"
                          << visual->material_name << " ' undefined!";
                throw URDFParseError(error_msg.str());
              }
            }
          }
