...
bool addChildrenToTree(std::shared_ptr<urdf::Link> root, KDL::Tree& tree) {
  ...
  // std::cout << "Link " << root->name.c_str() << " had " << children.size() <<
  // " children" << std::endl;
  ...
  KDL::Segment sgm(root->name, jnt,
                   toKdl(root->parent_joint->parent_to_joint_transform), inert);
...
bool treeFromUrdfModel(const urdf::UrdfModel& robot_model, KDL::Tree& tree) {
  ...
  if (robot_model.getRoot()->inertial) {
    std::cout << "The root link %s has an inertia specified in the URDF, but "
                 "KDL does not "
                 "support a root link with an inertia.  As a workaround, you "
                 "can add an extra "
                 "dummy link to your URDF."
              << robot_model.getRoot()->name.c_str() << std::endl;
  }
