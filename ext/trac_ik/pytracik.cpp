...
py::array_t<double> CartToJnt(TRAC_IK::TRAC_IK& t, py::array_t<double> init_q,
                              double x, double y, double z, double qx,
  ...
  for (int i = 0; i < init_q_buf.size; i++) q(i) = init_q_buf_ptr[i];

  int r = t.CartToJnt(q,
                      KDL::Frame(KDL::Rotation::Quaternion(qx, qy, qz, qw),
                                 KDL::Vector(x, y, z)),
                      result);
...
py::array_t<double> JntToCart(TRAC_IK::TRAC_IK& t, py::array_t<double> cfg) {
  ...
  for (int i = 0; i < cfg_buf_info.size; i++) q(i) = cfg_buf_ptr[i];
...
void setKDLLimits(TRAC_IK::TRAC_IK& t, const std::vector<double> lb,
