...
- ``"fk"`` — :class:`~TyGrit.robots.fetch.kinematics.fk_numpy.FetchSkeletonFK`
    All 15 link 4×4 poses, 10 joints (torso + 7 arm + 2 head).
    Pure NumPy, no C extension needed.

- ``"fk_ikfast"`` — :class:`~TyGrit.robots.fetch.kinematics.fk_ikfast.FetchIKFastFK`
    Gripper 4×4 only, 8 joints (torso + 7 arm).
    Uses IKFast C extension for speed.

- ``"fk_batch"`` — :class:`~TyGrit.robots.fetch.kinematics.fk_torch.FetchBatchFK`
...
def create_fetch_fk_solver(
    ...
    """
    if solver == "fk":
        from TyGrit.robots.fetch.kinematics.fk_numpy import FetchSkeletonFK

        return FetchSkeletonFK()

    if solver == "fk_ikfast":
        from TyGrit.robots.fetch.kinematics.fk_ikfast import FetchIKFastFK

        return FetchIKFastFK()

    if solver == "fk_batch":
        from TyGrit.robots.fetch.kinematics.fk_torch import FetchBatchFK
